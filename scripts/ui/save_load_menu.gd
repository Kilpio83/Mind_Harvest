...
@onready var fullscreen_button: Button       = $Panel/VBox/FullscreenButton
...
func _ready() -> void:
	...
	_save_btn.pressed.connect(func(): _on_name_confirmed(_name_input.text))
	_cancel_btn.pressed.connect(func(): _name_dialog.hide())
	_name_input.text_submitted.connect(_on_name_confirmed)
	fullscreen_button.text = "Fullscreen" if not SettingsManager.is_fullscreen() else "Windowed"
